...
impl<'a> Decoder<'a> {
    ...
    pub fn with_context(context: &'a mut zstd_safe::DCtx<'static>) -> Self {
        Self { context: MaybeOwnedDCtx::Borrowed(context) }
    }
...
impl Operation for Decoder<'_> {
    ...
    fn reinit(&mut self) -> io::Result<()> {
        match &mut self.context {
            MaybeOwnedDCtx::Owned(x) => x.reset(zstd_safe::ResetDirective::SessionOnly),
            MaybeOwnedDCtx::Borrowed(x) => x.reset(zstd_safe::ResetDirective::SessionOnly),
        }.map_err(map_error_code)?;
        Ok(())
    }
...
impl<'a> Encoder<'a> {
    ...
    pub fn with_context(context: &'a mut zstd_safe::CCtx<'static>) -> Self {
        Self { context: MaybeOwnedCCtx::Borrowed(context) }
    }
    ...
    pub fn set_pledged_src_size(
        ...
        match &mut self.context {
            MaybeOwnedCCtx::Owned(x) => x.set_pledged_src_size(pledged_src_size),
            MaybeOwnedCCtx::Borrowed(x) => x.set_pledged_src_size(pledged_src_size),
        }.map_err(map_error_code)?;
        Ok(())
    }
}
...
impl<'a> Operation for Encoder<'a> {
    ...
    fn reinit(&mut self) -> io::Result<()> {
        match &mut self.context {
            MaybeOwnedCCtx::Owned(x) => x.reset(zstd_safe::ResetDirective::SessionOnly),
            MaybeOwnedCCtx::Borrowed(x) => x.reset(zstd_safe::ResetDirective::SessionOnly),
        }.map_err(map_error_code)?;
        Ok(())
    }
}
